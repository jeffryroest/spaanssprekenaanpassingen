...
assert(progressSchema.$defs?.mission?.properties?.key?.enum?.includes('mission.madrid.review.personal'), 'Het voortgangscontract accepteert de persoonlijke herhaling niet');
...
assert(deck.includes('use ($attempt, $domainData, $releaseItem, $source, $steps, $scheduled)'), 'De kaartmapper ontvangt niet alle gepubliceerde broncontext');
