...
const [cssResponse, jsResponse, hubResponse, sceneResponse, luciaResponse] = await Promise.all([
...
if (!cssResponse.ok || !css.includes('.world-home-body') || !css.includes('.hub-world-return') || !css.includes('.bakery-lucia-frame')) {
  throw new Error('De live CSS is verouderd of onvolledig.');
}

if (!jsResponse.ok || !js.includes('madrid-mission-preparation') || !js.includes('npc_expression_sheet') || !js.includes('la-espiga-complete')) {
...
if (!sceneResponse.ok || !luciaResponse.ok) {
  throw new Error('Een meegeleverde gouden-route-afbeelding ontbreekt op de live site.');
}
