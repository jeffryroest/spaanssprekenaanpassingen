...
const paths = [
  ...
  'resources/views/components/player/navigation.blade.php',
...
const [registration, resetLink, resetPassword, account, accountOverview, authLayout, registerView, accountView, playerNavigation, accountsView, tests, adminTests, routes, provider, user, roadmap] = await Promise.all([
  read(paths[0]),
  read(paths[1]),
  read(paths[2]),
  read(paths[3]),
...
assert(user.includes('latestSubscription') && accountView.includes('<x-player.header') && playerNavigation.includes('Mijn proefweek'), 'De accountstatus is niet verbonden met toegang en abonnement');
